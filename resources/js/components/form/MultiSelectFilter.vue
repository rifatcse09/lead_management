<template>
  <div class="multi-select wrapper flex flex-col" :class="$attrs.class">
    ...
    <div class="wrapper relative" v-click-away="() => (expanded = false)">
      ...
      >
        <slot name="placeholder">
          <template v-if="!selected_items_labels.length">
            ...
            <div v-else class="placeholder" style="color: #4b4b4b99;" :class="$attrs.placeholderClass">
              {{ $t($attrs.placeholder ?? "") }}
            </div>
          </template>
          <template v-else>
            ...
            >
              <!-- {{ selected_items_labels.slice(0, 3).join(", ") }} -->
                <div style="color: #4b4b4b99;" :class="$attrs.placeholderClass" v-trans="$attrs.placeholder ?? ''">
                </div>
            </div>
      ...
      <div
        class="options absolute z-[1] bg-white flex flex-col w-full top-[99%] rounded-b-lg border border-input border-t-0 p-[14px] gap-[10px] max-h-[150px] overflow-y-auto"
...
const emit = defineEmits(["update:modelValue", "opened", 'scrolled']);
