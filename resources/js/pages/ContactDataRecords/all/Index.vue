<template>
    <div class="content px-6 min-w-[calc(100vw_-_320px)] w-[100vw_-_90px]">
       ...
       <div class="overview__tabs">
            ...
            <div v-if="hasPermission('contact-data-record:all-view')" class="overview__tabs--item active">{{ $t('All') }}</div>
       </div>
            ...
            <overview-table :paginationData="paginationData" :headerTop="hasAnyPermissions(['contact-data-record:allocation', 'contact-data-record:lead-again', 'contact-data-record:export'])">
                <template #header-top>
                    ...
                    <div class="flex gap-3.5">
                        ...
                        <LeadAgainButton class="w-max" :disabled="!enableSendLeadAgainButton" @click.prevent="showLeadAgainModal" v-if="hasPermission('contact-data-record:lead-again')">
                            ...
                            <ExportButton class="w-max" tab="all" v-if="hasPermission('contact-data-record:export')">
                            {{ $t("Export") }}
                        </ExportButton>
                    </div>
                </template>
                <template #header>
                    <div class="w-min flex">
                        ...
                        <SortColumn label="Category" columnName="category" style="width:220px;" />
...
<script setup>
    ...
    const getData = async ()=>{
        const {data} = await axios.get(route('contact-data-records.all.index', {_query: vueRoute.query}))
    ...
    const getOptionData = async (column) => {
        ...
        const {data} = await axios.get(route('contact-data-records.all.get-filters', {_query:filtersValues}))
    ...
    watch(
        ...
        async (newValue, oldValue) => {
            // console.log(vueRoute.name)
            if(vueRoute.name == 'contact-data-records-all-index') {
                getData()
            }
        }
    );
