<template>
    <div class="multi-select wrapper flex flex-col" :class="$attrs.class">
        <h2
            v-if="topLabel"
            class="text-[#AB326F] font-semibold text-[20px] pb-3"
        >
            {{ $t("Custom") }}
        </h2>
        <div
            v-if="!topLabel && hierarchyCustom"
            class="grow w-3/4 h-full"
        ></div>
        <h2
            class="text-input block mb-3 whitespace-nowrap"
            :class="$attrs.labelClass ?? 'text-formLabel'"
            v-if="$attrs.label"
        >
        ...
        <div class="wrapper relative"  v-click-away="() => (expanded = false)">
            <div
                ...
                :class="[
                    expanded && !disabled? 'rounded-b-[0px]' : '',
                    error ? '!border-error' : '',
                    $attrs.togglerClass,
                    disabled ? 'bg-[#FAFAFA]' : '',
                ]"
            ...
            >
                <slot name="placeholder">
                    <template
                        v-if="
                            searchables.length &&
                            expanded &&
                            !$attrs.hide_search_input
                        "
                    >
                        <input
                            class="search__input outline-none w-full h-full pl-[30px] text-[#636363] text-16"
                            ref="search_input"
                            type="text"
                            v-model="search"
                            @click.stop=""
                    ...
                    <template v-else-if="!selected_items_labels.length">
                        <div
                            ...
                            class="leading-8 text-bodyText/60"
                            :class="$attrs.placeholderClass ?? 'text-16'"
                        >
                            {{
                                $t(
                                    $attrs.placeholder.text,
                                    $attrs.placeholder.replace
                                )
                            }}
                        </div>
                        <div
                    ...
                    <template v-else>
                        <slot
                            name="selected-items-label"
                            :selected_items_labels="selected_items_labels"
                        >
                            <div
                        ...
                        <div
                            class="wrapper group relative"
                            v-if="selected_items_labels.length > 3"
                        >
                            ...
                            >
                                <div
                                    class="label flex gap-[10px] items-center text-[#636363] text-4 leading-4"
                                    v-for="(
                                        label, index
                                    ) in selected_items_labels.slice(
                                        3,
                ...
                <div class="toggle-icon ml-auto" v-if="!disabled">
            ...
            >
                ...
                <template
                    v-for="(
                        { [labelKey]: label, [valueKey]: value }, index
                    ) in options"
                    :key="index"
                >
                    <slot
                    ...
                    >
                        <div
                            class="option flex gap-[10px] items-center cursor-pointer w-[calc(100% - 14px)] break-all"
...
const emit = defineEmits(["update:modelValue", "opened", "onUpdate"]);
...
const toggle = () => {
    ...
    if (expanded.value) {
        ...
        setTimeout(
            () =>
                props.searchables.length ? search_input.value?.focus() : null,
            0
        );
    }
};
