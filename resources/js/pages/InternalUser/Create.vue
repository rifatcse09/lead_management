<template>
    ...
    >
        ...
        >
            ...
            <div
                class="wrappeer spacer col-span-3 grid grid-cols-3 gap-x-6 mb-[35px]"
                :class="[
            ...
            >
                ...
                <SingleSelect
                    ...
                    "
                />
                <!-- Custome  hierarcy lement as access right -->
                ...
                <!--Start None hierarcy by access right custom---->
                <template
                    v-if="
                        none_lavel_hierarchy_elements.length &&
                        form.access_right == 3 &&
            ...
            >
                ...
                <span
                    class="font-semibold font-inter text-[20px] text-[#AB326F] leading-[19px]"
                    >{{ $t("Send email invitation to Admin") }}</span
                >
            </div>
                ...
                <ButtonWhite
                    ...
                    @click="
                        $vfm.show('redirect-modal', {
                            title: 'Cancel registration of the internal user?',
                            description: `Internal user cancel registration description`,
                        })
                    "
                >
                    {{ $t("Cancel") }}
...
const addCompetencies = () => {
    const options = {
        ...
        on: {
            saveElement: (element) => {
                // console.log("competences", element);
...
const getNextPrefixId = async (model_name) => {
    try {
        const res = await axios.get(
            route("next-prefix-id", { modelName: model_name })
        );
...
const validateAcessRight = () => {
    let errorAccessRight = false;
    if (
        (form.roles_id == 1 || form.roles_id == 2) &&
        form.access_right.length == 0
    ) {
        form.errors.set(`access_right`, "Field Missing");
        errorAccessRight = true;
    }
    return errorAccessRight;
};
...
const submit = async () => {
    ...
    try {
        const res = await form.post(route("internal-users.store"));
        router.push({
            name: "internal-user-index",
        });
        const description =
            form.send_mail == true
                ? "The internal user has been successfully created and the email invitation has been sent."
                : "The Internal user was successfully created.";
        notificationSend.success(description);
    } catch (error) {
...
onBeforeMount(async () => {
    await getNextPrefixId("InternalUser");
    await getNoneLavelHierarchy();
    form.language_id = defaultLanguage.value.id;
});
</script>
