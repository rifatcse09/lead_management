<template>
  ...
  <script setup>
    ...
    const submit = async () => {
        ...
        try {
            ...
            const res = await axios.put(
                route("intermediaries.update", {
                    intermediary: brokerUser.id,
                }),
                data
            );
            router.push({ name: "intermediary-index" });
             notification.success({ text: 'The intermediary “:name” has been successfully updated.', replace: { name: res.data.user.full_name } })
            } catch (error) {
            console.log(error);
        }
  };
    ...
    const broker_name = ref(null)

    const getBrokerDetails = async () => {
        try {
            const res = await axios.get(
                route("intermediaries.show", {
                    intermediary: vueRoute.params.id,
                })
            );
            Object.assign(brokerUser, res.data);
            broker_name.value= brokerUser.broker.name;
    ...
    if( !permission.hasPermission('intermediares.edit')){
        router.push({ name: '403' })
    }
  </script>
