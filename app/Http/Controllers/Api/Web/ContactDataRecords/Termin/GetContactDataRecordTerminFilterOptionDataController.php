...
class GetContactDataRecordTerminFilterOptionDataController extends Controller
{
    use DropdownOption;
    ...
    public function index(Request $request)
    {
        ...
        if (strtolower(request('column')) ==   'birth_year') {
            // return $this->getOrganizationElementOptions($request);
        }
        ...
        if (request('column') ==   'control_status_appointment') {
            return $this->getControlStatusAppointmentOptions($request);
        }
     ...
     * @return void
     */
    protected function getControlStatusAppointmentOptions(Request $request){
        $search =  $this->getNestedFieldUniqueAggrations($request, 'last_appointment.control_status_appointment', 'last_appointment', true);
     ...
     * @return array
     */
    protected function getNestedFieldUniqueAggrations(Request $request, string $field, $parent, $keyword = false): array{
        $column = $keyword ? $field.'.keyword' : $field;
        $customer_company_id = auth()->user()->customer_company_id;
        ...
        if(isset($search['aggregations']) &&  isset($search['aggregations'][$parent]) &&  isset($search['aggregations'][$parent]['keys']) &&  isset($search['aggregations'][$parent]['keys']['buckets'])){
            $buckets = $search['aggregations'][$parent]['keys']['buckets'];
