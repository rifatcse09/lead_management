<template>
    ...
    >
        ...
        <div class="filter__menu absolute" v-if="openFilterMenu" :style="{width: width}">
            <div class="filter__menu--header mb-8 flex">
                <div class="tabs flex gap-9">
                    <!-- <button class="tab_button" :class="{active: tab == 'filter'}" @click.prevent="tab = 'filter'">{{ $t('Filter') }}</button>
                    <button class="tab_button" :class="{active: tab == 'advanced_filter'}" @click.prevent="tab = 'advanced_filter'">{{ $t('Advanced Filter') }}</button> -->
                </div>
                <div class="close_icon ml-auto">
                    <slot name="close">
            ...
            <div class="filter__menu--body">
                ...
                <!-- <slot name="advanced_filter" :key="componentKey" v-if="tab == 'advanced_filter'"></slot> -->
            </div>
            <div class="filter__menu--footer flex w-full gap-3 mt-[40px]">
                <slot name="footer">
                    <button
                        class="filter__apply--button font-inter text-white text-center rounded-[10px] px-[30px] py-[12px] font-medium  w-[142px]"
                        @click="apply"
                    >
                        {{ $t('Apply') }}
                    </button>
                    <button
                        :class="`filter__reset--button font-inter text-white text-center rounded-[10px] ${reset=='Reset' ? 'px[30px]' : 'px[15px]'} py-[12px] font-medium  w-[142px]`"
                        @click="reset"
                    >
                        <div>{{ $t('Reset') }}</div>
                    </button>
  ...
  <style lang="scss" scoped>
      ...
      .tab_button {
        ...
        .tab_button.active {
            border-bottom: 1px solid #8B387F;
            pointer-events: none;
        }
  </style>
