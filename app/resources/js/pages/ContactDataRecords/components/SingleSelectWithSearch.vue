<template>
      ...
      >
        ...
        >
          <slot name="placeholder">
            <template v-if="searchable && !hide_search_input">
              <input
            ...
            <template v-else-if="selected_item_label === null">
              <div class="text-[16px] leading-8 text-bodyText/60 text-ellipsis overflow-hidden whitespace-nowrap">
                {{ $t($attrs.placeholder ?? "") }}
              </div>
            </template>
            <template v-else>
        ...
        <div
          ...
          v-if="expanded"
        >
        <div class="pl-[10px] pr-[10px] mt-[10px] mb-[10px]" v-if="hide_search_input">
          <input class="search__input outline-none w-full h-full text-[#636363] text-16 pl-[30px]" ref="search_input" type="text" v-model="search" @click.stop="" />
        </div>
          <template
            v-if="options.length"
            v-for="{ [labelKey]: label, [valueKey]: value, ...additionals } in options"
          >
            <slot
              name="option"
              :option="{
            ...
            >
              ...
              >
                <span class="text-[16px] leading-4">{{ $t(label.toString()) }}</span>
              </div>
            </slot>
          </template>
        </div>
  ...
  const selected_item_label = computed(() => {
    const selected_options_label = props.options.find((item) => {
      if (typeof attrs.modelValue == "string" && typeof item[props.valueKey] == "string") {
        return item[props.valueKey].toLowerCase() === attrs.modelValue.toLowerCase();
      }
      return item[props.valueKey] === attrs.modelValue;
    });
    return selected_options_label ? selected_options_label[props.labelKey] : null;
  });
    ...
    return props.options.filter((option) => {
      if (props.searchables.length) {
        return props.searchables.some((searchKey) =>
          option[searchKey].toLowerCase().startsWith(search.value.toLowerCase())
        );
      } else {
        return option[props.valueKey].toLowerCase().startsWith(search.value.toLowerCase());
      }
    });
  });
