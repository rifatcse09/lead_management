    ...
    <div class="content px-6 min-w-[calc(100vw_-_320px)] w-[100vw_-_90px]">
       <div class="overview__header mb-[38px]">
            <div class="overview__header--title">
                <template v-if=" userStore.user.type == 'internal_user' && userStore.user.role == 'Call agent' ">
                    <h3 v-if="userStore.user.alignment.includes('1') && !userStore.user.alignment.includes('2')"> {{ $t('Leads') }}</h3>
                    <h3 v-else>{{ $t('Contact Data Records') }}</h3>
                </template>
                <template v-else-if=" userStore.user.type == 'internal_user' && userStore.user.role == 'Quality controller' ">
                    <h3 v-if="userStore.user.alignment.includes('1') && userStore.user.alignment.includes('2')">
                        {{ $t('Contact Data Records') }}
                    </h3>
                    <h3 v-else-if="userStore.user.alignment.includes('1')">{{ $t('Leads') }}</h3>
                </template>
                <template v-else>
            ...
            <div class="overview__header--content flex justify-between">
                <div class="overview__header--left w-[830px]">
                    <Search placeholder="Search by ID, first-, last name, mobile phone number, email, street name, zip code or canton" />
                </div>
       ...
       <div class="overview__tabs">
            <div  v-if="hasPermission('contact-data-record:lead-view')" class="overview__tabs--item active">{{ $t('Leads') }}</div>
            <router-link  v-if="hasPermission('contact-data-record:termin-view')" :to="{name: 'contact-data-records-termin-index'}" class="overview__tabs--item">{{ $t('Termine') }}</router-link>
            ...
            <overview-table :paginationData="paginationData" :headerTop="hasAnyPermissions(['contact-data-record:allocation', 'contact-data-record:lead-again', 'contact-data-record:export'])">
                <template #header-top>
                    ...
                    <div class="flex gap-3.5">
                        <!-- <AllocateButton class="w-max" :disabled="!selectedRows.some((item)=>item)">
                            {{ $t("Allocate Leads") }}
                        </AllocateButton> -->
                        ...
                        <ExportButton class="w-max" tab="leads"  v-if="hasPermission('contact-data-record:export')">
                            {{ $t("Export") }}
                        </ExportButton>
                    </div>
                </template>
                <template #header>
                    <div class="w-min flex">
                        ...
                        <SortColumn label="ID" columnName="prefix_id" style="width:110px;" />
                        <SortColumn label="Creation Date" columnName="created_at" style="width:190px;" />
                        <SortColumn label="Creator" columnName="user_id" style="width:180px;" :enable="false"/>
                        ...
                        <SortColumn label="Year" columnName="date_of_birth" style="width:190px;"/>
                        <SortColumn label="Last Feedback" columnName="feedback" style="width:190px;"/>
                        <SortColumn label="Feedback Time" columnName="feedback_time" style="width:200px;"/>
                        <SortColumn label="Control status" columnName="lead" style="width:225px;"/>
...
<script setup>
    ...
    const getData = async ()=>{
        const {data} = await axios.get(route('contact-data-records.index', {_query: vueRoute.query}))
    ...
    const getOptionData = async (column) => {
        ...
        const {data} = await axios.get(route('contact-data-records.leads.get-filters', {_query:filtersValues}))
    ...
    watch(
        ...
        async (newValue, oldValue) => {
            if(vueRoute.name == 'contact-data-records-leads-index') {
                getData()
            }
        }
    );
