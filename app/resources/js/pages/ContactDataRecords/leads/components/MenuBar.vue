<template>
  <div class="table__menubar justify-start" v-click-away="() => (openDropdownMenu = false)">
    ...
    <ul class="table__menubar__dropdown" v-if="openDropdownMenu">
      <!-- <li>
        <router-link :to="{ name: 'customer-company-show', params: { id: contact_data_record.id } }">{{ $t('Set as Lead again') }}</router-link>
      </li> -->
      ...
      <li>
        <router-link :to="{ name: 'contact-data-records-show', params: { id: contact_data_record.id } }">{{ $t('View Details') }}</router-link>
      </li>
      <!-- Check duplicate will implament later -->
      <li v-if="status == 'Check Duplicate' || status == 'Duplicate'">
        <router-link :to="{ name: 'contact-data-records-show', params: { id: contact_data_record.id }, query: { duplicate_check: true } }">{{ $t('Check Duplicate') }}</router-link>
      </li>
        <template v-if=" userStore.user.type == 'internal_user' && userStore.user.role == 'Call agent' ">
            <li v-if="userStore.user.alignment.includes('1') && !contact_data_record.lead ">
                <router-link :to="{ name: 'contact-data-records-edit', params: { id: contact_data_record.id } }" >{{ $t('Edit') }}</router-link>
            </li>
            <!-- <li v-if="userStore.user.alignment.includes('2') && !contact_data_record.lead && contact_data_record.status != 'terminated' &&  contact_data_record.status != 'Quality topic solved'">
                <router-link :to="{ name: 'contact-data-records-edit', params: { id: contact_data_record.id } }" >{{ $t('Edit') }}</router-link>
            </li> -->
        </template>
        ...
        <template v-if=" userStore.user.type == 'internal_user' && (userStore.user.role == 'Leader' || userStore.user.role == 'Manager') ">
            <li>
                <router-link :to="{ name: 'contact-data-records-edit', params: { id: contact_data_record.id } }">{{ $t('Edit') }}</router-link>
            </li>
        </template>
        <template v-if=" userStore.user.type == 'company_admin'">
            <li>
                <router-link :to="{ name: 'contact-data-records-edit', params: { id: contact_data_record.id } }">{{ $t('Edit') }}</router-link>
            </li>
        </template>
      ...
      <li class="flex whitespace-nowrap h-[25px] items-center px-[14px] cursor-pointer" @click="assignAppointment" v-if="contact_data_record.contact_record_status == 'Confirmed'">
        {{ $t('Enter new appointment') }}
      </li>
      <li v-if="contact_data_record.contact_record_status != 'Duplicate' && contact_data_record.contact_record_status != 'Check Duplicate' && hasPermission('contact-data-record:allocation') ">
        <a @click.prevent="allocate">{{ $t("Allocate Leads") }}</a>
      </li>
...
import AssignAppointmentModal from '../../components/AssignAppointmentModal.vue'
