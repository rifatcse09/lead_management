<template>
    <div class="multi-select wrapper flex flex-col" :class="$attrs.class">
      ...
      >
        <template v-if="typeof $attrs.label == 'object'">
          {{ $t($attrs.label.text, $attrs.label.replace) }}{{ $attrs.asterisk ? "*" : ""
      ...
      <div class="wrapper relative" v-click-away="() => (expanded = false)">
        ...
        >
          <slot name="placeholder">
            <template v-if="searchables && expanded && !$attrs.hide_search_input">
              <input
  ...
  const selected_items_labels = computed(() => {
    const selected_options_labels = translatedLanguages
      .filter(({ ['code']: value, ['name']: label }) => {
  ...
  const toggle = () => {
    ...
    if (expanded.value) {
      emit("opened");
      setTimeout(() => (props.searchables ? search_input.value?.focus() : null), 0);
    }
  };
