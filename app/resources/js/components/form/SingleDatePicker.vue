<template>
    <div class="input-element w-full" :class="$attrs.wrapperClass">
        <label :for="$attrs.id" class="text-input block mb-3 text-formLabel" :class="$attrs.labelClass" v-if="label">
            <template v-if="typeof $attrs.label == 'object'">
                {{ $t(label.text, label.replace) }}
            </template>
            <template v-else> {{ $t(label) }}{{ $attrs.asterisk ? '*' : '' }} </template>
        </label>
        <div class="datepicker" :class="{ 'border-error': error }">
            <v-date-picker v-model="date_value" title-position="left" @dayclick="dayclick"
                :attributes="datepickerOptions.attrs" :locale="userStore.user.language.code" :select-attribute="{
                    highlight: {
                        start: { fillMode: 'solid', style: { background: '#9E3576' } },
                        base: {
                            fillMode: 'light',
                            contentStyle: { color: '#ffffff' },
                            style: { background: '#C73B81' },
                        },
                        end: { fillMode: 'solid', style: { background: '#9E3576' } },
                    },
                }" :drag-attribute="{
    highlight: {
        start: {
...
}" :key="compoenentKey" :masks="{ input: 'DD.MM.YYYY', title: 'MMM YYYY' }" :model-config="{
    type: 'string',
    mask: 'YYYY-MM-DD', // Uses 'iso' if missing
}" :min-date="$attrs.min_date">
                <template v-slot="{ inputValue, inputEvents }">
                    <div class="filter-date-range-picker h-10 items-center relative">
                        <input id="date" class="date__input" :placeholder="$t('dd.mm.yyyy')" :value="inputValue"
                            v-on="inputEvents" />
                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"
                            class="ml-auto absolute right-0 pointer-events-none" @click="focusDateInput">
                            <path
        ...
        <p class="error-box flex p-0 items-center gap-2 mt-[4px] text-error text-12"
...
.datepicker.border-error {
    border: 1px solid rgb(249 50 50 / var(--tw-border-opacity));
}
