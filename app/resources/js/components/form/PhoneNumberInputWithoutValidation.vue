<template>
  <div class="wrapper">
    ...
    <div class="w-full relative h-max" v-bind="attrs" v-click-away="() => (expand = false)">
      <div class="flex h-10 w-full bg-white rounded-[8px] gap-6 items-center px-3 border border-input" :class="[error ? '!border-error' : '', expand ? 'rounded-b-[0px]' : '', attrs.class]">
        <div class="country-code-toggler w-max" @click="expand = true">
          <div v-if="!expand && selected_country" class="selected-code flex gap-3 items-center justify-center text-input">
            <div class="default-flat iti-flag" :style="{ backgroundImage: `url(${FlagImage})` }" :class="selected_country.iso"></div>
            <div>+{{ selected_country.value }}</div>
          </div>
      ...
      <div class="country-codes absolute z-10 bg-white flex flex-col w-full top-[99%] rounded-b-lg border-b-[1px] border-x-[1px] border-input max-h-[150px] overflow-y-auto" v-if="expand">
        ...
        >
          <div class="wrapper flex w-6 justify-center items-center mr-3">
            <div class="default-flat iti-flag" :style="{ backgroundImage: `url(${FlagImage})` }" :class="`${country.iso}`"></div>
          </div>
          <div class="dialcode">+{{ country.value }}</div>
...
const number = computed({
  ...
  set: function (value) {
    emit('update:phone_number', value.replace(/[^0-9]/g, ''))
    emit('updated', value)
  },
})
</script>
