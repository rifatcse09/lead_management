...
trait GetAllTabDataOpenSearchQuery {
    use OrderByDirection;

    private $indexName = ContactDataRecord::OPENSEARCH_INDEX_NAME;


    public function getAllTabRecords(Client $client,  $current_page, $per_page, array $additional_queries = []){
        ...
        $search =  $client->search([
            ...
            'body' => $this->buildAllTabQuery($per_page, $current_page, $additional_queries)
        ]);
    ...
    private function buildAllTabQuery($per_page, $current_page, $additional_queries){
                ...
                $filters[]= [
                    'bool'=> [
                        "must_not" => [
                            [
                                "terms" => [
                                    "contact_record_status" => ["Duplicate", "Check Duplicate"],
                                ],
                            ],
                        ],
                    ]
        ...
        return [
            ...
            'sort'=> $this->generateAllTabSortQuery(),
    ...
    protected function generateAllTabSortQuery(){

        $order_by = 'created_at';
        $order_direction = strtolower(request('direction')) == 'asc' ? 'asc': 'desc';

        if(request()->has('order_by') && !is_null(request('order_by'))) {
            if(in_array(request('order_by'), ['prefix_id', 'category', 'first_name', 'last_name', ])){
